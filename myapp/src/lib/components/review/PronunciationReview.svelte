<script lang="ts">
	...
	const audioText = $derived(String(content.audio_text ?? content.back ?? ''));
...
<div class="pronunciation">
	<div class="card">
		...
		{:else}
			<p class="instruction">Say the answer out loud, then reveal.</p>
		{/if}
		<p class="prompt">{front}</p>

		{#if showAnswer}
			<div class="answer">
				<p class="answer-label">Target</p>
				<p class="target">{back}</p>
				{#if audioText}
					<p class="hint">Shadow the audio:</p>
					<TtsPlayButton text={audioText} label="Play model" />
				{/if}
			</div>
		{/if}
	</div>
